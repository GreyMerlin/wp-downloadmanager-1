...
function downloads_css() {
	...
	if(@file_exists(TEMPLATEPATH.'/download-css.css')) {
		echo '<link rel="stylesheet" href="'.get_stylesheet_directory_uri().'/download-css.css" type="text/css" media="screen" />'."\n";	
	} else {
		echo '<link rel="stylesheet" href="'.WP_PLUGIN_URL.'/wp-downloadmanager/download-css.css" type="text/css" media="screen" />'."\n";
	}	
	echo '<!-- End Of Script Generated By WP-DownloadManager 1.31 -->'."\n";
}
...
function download_tinymce_addplugin($plugin_array) {
	$plugin_array['downloadmanager'] = WP_PLUGIN_URL.'/wp-downloadmanager/tinymce/plugins/downloadmanager/editor_plugin.js';
	return $plugin_array;
}
...
function downloads_page($category_id = 0) {
	...
	if($max_page > 1) {
		...
		$output .= '<span class="pages">'.sprintf(__('Page %s of %s', 'wp-downloadmanager'), $page, $max_page).'</span>';			
		...
		for($i = $start_page; $i  <= $end_page; $i++) {						
			if($i == $page) {
				...
				$output .= '<a href="'.download_page_link($i).'" title="'.$i.'">'.$i.'</a>';
			}
		}
...
function create_download_table() {
	...
	add_option('download_path_url', WP_CONTENT_URL.'/files', 'Download Path URL');
	...
	add_option('download_template_listing', array('<p><img src="'.WP_PLUGIN_URL.'/wp-downloadmanager/images/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong><a href="%FILE_DOWNLOAD_URL%">%FILE_NAME%</a></strong><br /><strong>&raquo; %FILE_SIZE% - %FILE_HITS% '.__('hits', 'wp-downloadmanager').' - %FILE_DATE%</strong><br />%FILE_DESCRIPTION%</p>', '<p><img src="'.WP_PLUGIN_URL.'/wp-downloadmanager/images/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong>%FILE_NAME%</strong><br /><strong>&raquo; %FILE_SIZE% - %FILE_HITS% '.__('hits', 'wp-downloadmanager').' - %FILE_DATE%</strong><br /><i>'.__('You need to be a registered user to download this file.', 'wp-downloadmanager').'</i><br />%FILE_DESCRIPTION%</p>'), 'Download Listing Template');
	add_option('download_template_embedded', array('<p><img src="'.WP_PLUGIN_URL.'/wp-downloadmanager/images/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong><a href="%FILE_DOWNLOAD_URL%">%FILE_NAME%</a></strong> (%FILE_SIZE%, %FILE_HITS% '.__('hits', 'wp-downloadmanager').')</p>', '<p><img src="'.WP_PLUGIN_URL.'/wp-downloadmanager/images/%FILE_ICON%" alt="" title="" style="vertical-align: middle;" />&nbsp;&nbsp;<strong>%FILE_NAME%</strong> (%FILE_SIZE%, %FILE_HITS% '.__('hits', 'wp-downloadmanager').')<br /><i>'.__('You need to be a registered user to download this file.', 'wp-downloadmanager').'</i></p>'), 'Download Embedded Template');
